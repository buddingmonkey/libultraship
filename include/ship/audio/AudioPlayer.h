...
class AudioPlayer {
  ...
  public:
    ...
    /** @brief Releases the output device without tearing the player down; Resume() reclaims it. */
  ...
  protected:
     ...
     *
     * Unlike SetAudioChannels() this does not call DoClose()/DoInit(), so it is safe to call from
     * within DoInit(). The saved channel preference is left untouched.
     *
